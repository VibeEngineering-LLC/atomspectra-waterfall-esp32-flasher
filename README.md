...
1. Запустить `atomspectra-waterfall-esp32-flasher-X.Y.Z.exe` (скачать с
   [Releases](https://github.com/VibeEngineering-LLC/atomspectra-waterfall-esp32-flasher/releases/latest))
...
5. Для AtomSpectra (водопад) — опционально указать IP платы и оставить включённым
   «Перезагрузить плату по сети перед прошивкой»: плата успеет корректно закрыть открытый
   сегмент водопада до аппаратного сброса esptool. У остальных проектов этого шага нет.
6. Нажать «Установить»

Flasher автоматически загрузит выбранную версию прошивки с GitHub и прошьёт плату.
...
в эфире сам. Прибор можно подключить и USB-кабелем: прошивка сама выбирает канал (USB, если прибор воткнут,
иначе BLE); для питания прибора по USB на плате нужна замкнутая перемычка USB-OTG (подробности — в README
прошивки). Порядок шагов программа печатает после успешной прошивки.
...
# → dist/atomspectra-waterfall-esp32-flasher-X.Y.Z.exe (версия — из flasher/__init__.py)
```
